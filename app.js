...
// "New Release" isn't a real decade — it's a rolling window (movies released
// in the last ~6 months). Shown as an extra era button alongside DECADES,
// but a movie can match it *in addition to* its normal decade, not instead of.
const ERA_BUTTONS = [...DECADES, { key: "new", label: "🆕 New Releases" }];
...
const CACHE_KEY = "movieNightCache_v3"; // bumped: schema now includes releaseDate (for New Release filter)
...
const RECENT_RELEASE_WINDOW_MS = 1000 * 60 * 60 * 24 * 182; // ~6 months, in fixed days to avoid Date.setMonth's day-of-month rollover

function isRecentRelease(movie) {
  if (!movie.releaseDate) return false;
  const cutoff = Date.now() - RECENT_RELEASE_WINDOW_MS;
  return new Date(movie.releaseDate).getTime() >= cutoff;
}
...
function matchesFilters(movie) {
  ...
  if (selectedDecades.size > 0) {
    ...
    const matchesEra =
      selectedDecades.has(decade) || (selectedDecades.has("new") && isRecentRelease(movie));
